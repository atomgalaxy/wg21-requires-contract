...
> [@P2388R2] has no answer for *precondition*s that need to mutate a copy:
...
the binding type (reference or copy?) in either of the possibilities.
...
That said, putting parentheses around the expression after the `=>` restores
readability, and in addition screams _this is an expression_.
