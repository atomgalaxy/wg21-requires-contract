...
were immediately-invoked lambda expressions, and are therefore are not a problem.
...
part of _B_ shall not be executed before _A_ has been checked; This restriction
...
it doesn't have an obvius closure.
...
This section explores future extensions as invisaged by [@P2388R2] and previous papers.

## Place for annotations like "axiom", "new", etc.

The best idea for where to put such markers it at the end, after a semicolon; from [@P2388R2]/8:
...
[@P2388R2] side-steps this issue by saying that referencing aruments which changed value is UB, which is why referenced arguments should be `const`-qualified (in definitions).
