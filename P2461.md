...
The attribute-derived syntax for contracts is limiting and steps on the shared
space between C and C++. This paper explores an alternative syntax that should
offer an eaasier extension path.
...
**Note:** this paper is an exploration. The authors do not object to the
currently agreed-upon syntax; but it does seem to present certain challenges
that this paper tries to address.
...
is valid within the _correctness-specifier-body_.
...
We need to make preceding _precondition_\ s protect both the _lambda-introducer_
...
`pre`-conditions; their _bodies_ are, obviously, evaluated after the function
exits.

If a `pre`-condition _B_ follows a `pre`-condition _A_ in a function's declaration,
then no part of _B_ shall be executed before _A_ has been proven;

If a `post`-condition _P_ follows a `pre`-condition _A_ in a function's
...
refer to rvalue-reference arguments, and all the other possible
implementation-limitations as well.

### Capturing views by value
...
An alternative (although the authors are not completely sure this does not conflict with the _attribute-specifier-seq_ that appertains to the function type):
...
This is due to
  ...
  - enabling contracts should ideally not _introduce additional bugs_.
  - contracts necessarily _narrow_ from the _wide_ contract specification; therefore the contract specification of the *check* is wider than the contract of the *function*.
...
Postconditions *have* to be fuzzable independently of their functions. If the
...
- The closure-based syntax makes it obvious when values are captured, and even
  hints an implementation - just put the closures on the stack before the
  function arguments.
...
- **(!)**: Capturing by value in postconditions makes no sense to the caller,
  therefore it's not a well-behaved contract and we should diagnose (2).
...
# Considered and rejected syntaxes
...
`post`-conditions is identical to the `&&`-operator, because we want short-circuiting. In addition, the lack of a `return` keyword in the body makes the individual conditions seem disembodied.
