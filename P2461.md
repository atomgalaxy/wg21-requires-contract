...
hopefully be more powerul while being able to express the same semantics.
...
`pre` and `post` are only keywords in the toplevel context of a function declarator.
In the future, we can put other keywords between the keyword and the colon (see example).

`pre` and `post` can appear in function declarations after the place for the
optional trailing `requires` clause.
...
|   _lambda-introducer~opt~_ _return-value-id~opt~_
...
The lambda-closure definition works with this - the function arguments are
...
## We can have attributes appertaining to contract annotations
...
A vendor of compiler extensions can always ship their own features as
attributes - but this would not go quite as smoothly with almost-attributes.
